...
public class Branje {
	...
	public static void main(String[] args) throws IOException {
		...
		for (String kljuc : zbirkaBesed) { // Izpis besed v slovarju
	...
	public static Set<String> neStejemo(String imeVhod) throws IOException {
		...
		int stevec = 0;
		while (vhod.ready()) {
			String vrstica = vhod.readLine().trim(); // .trim() je kot .strip()
			if (vrstica.equals(""))
				continue;
			besede.add(vrstica);
			++stevec;
	...
	public static Map<String, Integer> deloZdatotekami(String imeVhod) throws IOException {
		...
		while (vhod.ready()) {
			...
			if (vrstica.equals(""))
				continue;
			StringTokenizer st = new StringTokenizer(vrstica, " '?!.,;\""); // Teh znakov ne bomo upoštevali
			while (st.hasMoreTokens()) {
				...
				if (!nesteteBesede.contains(beseda)) {
					...
					if (vrednost == null)
						vrednost = 0;
					besedisce.put(beseda, vrednost + 1);
				} else {
					++stIzkljucenih;
				}
			}
